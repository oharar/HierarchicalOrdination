...
# "RW" can be used to specify parameters on which to apply univariate Metropolis-Hastings
# parameters that are not in "block" or "RW" are HMCed
...
Now that we have two or more latent variables, we also have to worry about their rotation. Previously, we fixed some parameters to fix the rotation. That has the downside of having to set the number of latent variables *a-priori*. Here, we instead order the LVs by their associated variation as in classical ordination methods, and add shrinkage to make sure that the maximum amount of variation is explained by each of the latent variables. <details><summary>For those who are interested, this is explained further here.</summary>
First, we introduce the $d-$sized vector $\symbf{\xi}$. We assume the following prior structure for this vector: $\xi_1 \sim \Gamma(5, 2)$ and $\xi_{q \in d \setminus \{1\}} \sim B(1,10)$. We parameterize the LV variation parameters as: $\Sigma_{q,q} = \prod^d_{q=1} \delta_{1:q}$, so that each latent variable explains less variation than the next. This shows that the choice for the parameterization of the gamma distribution is to be uninformative, since we do not know how much variation the latent variables explain in the data (but usually, this ranges between 1-5). The choice of parameterization for the beta distribution is motivated as a shrinkage prior; with a low mean and large variance, the variation of the first (and each consecutive) latent variable is forced to be as high as possible, but considerably lower than that of the previous latent variable.
</details>
...
HO.nim <- nimbleCode({
  for (i in 1:NSites) {
    ...
    for (l in 1:nLVs) {
      ...
      epsilonSTAR[i,l] ~ dnorm(0,1)# Residual
      zSTAR[i,l] <- XB[i,l] + sd.Site[l]*epsilonSTAR[i,l]
    }
  }
  ...
  for(j in 1:NSpecies) {
    for (l in 1:nLVs) {
      ...
      varepsilonSTAR[j,l] ~ dnorm(0,1) # Residual
      gammaSTAR[j,l] <- omegaTR[j,l] + sd.Species[l]*varepsilonSTAR[j,l]
    }
  ...
  xi[1] ~ dgamma(5,2)
  for(l in 2:nLVs) {
  xi[l] ~ dbeta(1,10)#dbeta(NSpecies/(2*l+NSpecies), l^2) # could instead be dunif(0,1) to be less informative
  }
...
#                       Nburn=5e1, NIter=5.5e2, Nthin=1, # for a small run
                       ...
                       consts = consts, block = MakeBlockList(consts), RW = c(paste0("beta0[",1:consts$NSpecies,"]"))) # HMC on the rest (hypers)
```
