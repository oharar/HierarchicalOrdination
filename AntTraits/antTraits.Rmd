...
#    the Bs are the coefficients of the environmental effects
      epsilon[i] ~ dnorm(0, sd = sd.Site) # Residual site effect
...
#    The Os are the trait effects.
      varepsilon[j] ~ dnorm(0, sd = sd.Species) # Residual
...
RunOneChain <- function(seed, dat, code, inits, consts, ToMonitor=NULL, 
  ...
  HMCsamplers <-c('epsilon', 'varepsilon','B', 'O', 'beta0', 
...
#  Mons <- c("beta0","sd.Species","sd.Site","sd.LV","B","O", "epsilon","varepsilon")
...
HO.nim <- nimbleCode({
  for (i in 1:NSites) {
    ...
    for (l in 1:nLVs) {
      XB[i, l] <- sum(X[i, 1:NEnv]*B[1:NEnv, l])
      epsilon[i,l] ~ dnorm(0,sd.Site[l])# Residual
      z[i,l] <- XB[i,l] + epsilon[i,l]
    }
  }
  
  for(j in 1:NSpecies) {
    for (l in 1:nLVs) {
      omegaTR[j, l] <- sum(TR[j, 1:NTraits]*O[1:NTraits, l])
      varepsilon[j,l] ~ dnorm(0,sd.Species[l]) # Residual
      gamma[j,l] <- omegaTR[j,l] + varepsilon[j,l]
    }
...
rotate2LV <- function(ch, RotateBy, SDby, ToRotate=NULL, SDToRotate=NULL, rotation.fn="svd", 
  ...
  if(scale){
    ...
    ch[grep(paste0("^", RotateBy, "\\["), names(ch))]  <- scale(LVmat)
     ch[grep(paste0("^", SDby, "\\["), names(ch))] <- attr(scale(LVmat),"scaled:scale")
  } else{
   # if LVs are not scaled by LV sds, we just add the new LV scale to LVs SDs.
     ch[grep(paste0("^", RotateBy, "\\["), names(ch))] <- attr(scale(LVmat),"scaled:scale")
     ch[grep(paste0("^", SDby, "\\["), names(ch))] <- attr(scale(LVmat),"scaled:scale")*SDs
  }
...
HO.2LV <- ParaNimble(4, fun = RunOneChain,
                       ...
                       inits = inits, HMC=TRUE,
#                       Nburn=5e1, NIter=5.5e2, Nthin=1, # for a small run
                       Nburn = 1e3, NIter = 4e3, Nthin = 1, # for a full run
                       consts = consts)#, HMCsamplers = c("sd.LV", "sd.Site", "sd.Species")) # HMC on hypers
