...
rotate2LV <- function(ch, RotateBy, SDby, ToRotate=NULL, SDToRotate=NULL, rotation.fn="svd", 
  ...
  getLVsd <- function(vec, name, RotateBy) {
     LVmat <- ChainToMatrix(vec, RotateBy)
     vec[grep(paste0("^", name, "\\["), names(vec))] <- attr(scale(LVmat),"scaled:scale")
     return(vec)
  }
  ...
  loadings.r <- list(Th=svd(LVmat)$v)
  }
  # Rotate variables
  for(nm in unique(c(RotateBy, ToRotate))) {
    ch <- RotateVar(vec=ch, name=nm, rotmat=loadings.r$Th, retmat = FALSE)
  }
  # SDs are not rotated, but are now retrieved from either z or gamma (whichever we rotated by)
  # Rotation of epsilon and varepsilon doesn't affect their SDs anyway
  # Scaling does
  # for(nm in unique(c(SDby, SDToRotate))) {
  #   ch <- RotateSD(vec=ch, name=nm, rotmat=loadings.r$Th)
  # }
  ch <- getLVsd(vec=ch, name = "sd.LV", RotateBy = RotateBy)
  
  # I assume Z and Gamma have already been standardized previous to this function, so I leave those as they are.
  # Only thing remains is to return the chain
  ch
}
...
# post-process chains for sign-swapping
chains2LV <- postProcess(chains2LV.r, VarsToProcess=VarsToProcess, 
