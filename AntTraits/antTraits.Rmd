...
rotate2LV <- function(ch, RotateBy, SDby, ToRotate=NULL, SDToRotate=NULL, rotation.fn="Varimax", 
  ...
  for(nm in unique(c(RotateBy, ToRotate))) {
    ch <- RotateVar(vec=ch, name=nm, rotmat=t(loadings.r$Th), retmat = FALSE)
  }
  for(nm in unique(c(SDby, SDToRotate))) {
    ch <- RotateSD(vec=ch, name=nm, rotmat=t(loadings.r$Th))
  }
  ch
}
...
# Varimax rotation
# scale = TRUE so that Z*Sigma gets rotated. Otherwise just Z.
chains2LV.r <- RotateChains(chains2LV.sc, rotation.fn="Varimax", scale = T)

# Now scale LVs
chains2LV.sc2 <- RescaleChains(chains2LV.r, ScaleBy="z", 
                              SDTorescale=c("sd.LV", "sd.Site"), 
                              ToRescale=c("z",  "B", "epsilon"))

# post-process chains for sign-swapping
chains2LV <- postProcess(chains2LV.sc2, VarsToProcess=VarsToProcess, 
