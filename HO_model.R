...
#  Give N(0,1) priors, so they are on about the same scale as the epsilons
...
# Infinite factor latent variable method.
a1 <- gamma(2, 1, dim = 1)
a2 <- gamma(2, 1, dim = 1)
...
# z.sd <- applyVar(z, MARGIN = 2, sd=TRUE)
# gamma.sd <- applyVar(gamma, MARGIN = 1, sd=TRUE)
# LVscales <- LVscales.unsc*z.sd*gamma.sd
