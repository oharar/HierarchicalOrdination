...
subtitle: "(fourth) cornering the market in modelling communities"
author: 
  - R.B. O'Hara$^{12}$
  - B. van der Veen$^{123}$
...
This model can be extended in many ways, as described below. The strategy for doing this is to include further effects for the sites, species and latent effects, i.e. $\alpha_i$, $\phi_j$, $z_{il}$, and $\gamma_{jl}$. 
...
\begin{equation}
\eta_{ij} = \sum \limits^{K}_{k=1} \beta_{jk}X_{ik} + \sum \limits^{S}_{s=1}\theta_{is}W_{js}  + \sum \limits_{l=1}^L z_{il}^T \gamma_{jl},
\end{equation}
...
Random effects can also be added. In essence, this can be interpreted as simply putting a covariance structure on some of the $\beta_{jk}$'s or $\theta_{is}$'s. For example, a phylogenetic effect can be added by having a $\theta_{is}$ for each species, but forcing a phylogenetic correlation, i.e. species with a more recent common ancestor tend to have more similar values of $\theta_{is}$.

Classical ordination takes a different approach to including covariates. Instead of the site main effects being modeled, covariates are included through **constrained ordination** [e.g. @terBraak86CCA], where the site scores of the latent variables are forced to be linear functions of covariates, i.e. 
...
&         = \alpha_i + \phi_j +\boldsymbol{x}_{i}^\top \boldsymbol{\psi} \Lambda \boldsymbol{\delta}^\top\boldsymbol{w}_{j} + \boldsymbol{\varepsilon}_{i}^\top \Lambda \boldsymbol{\delta}^\top\boldsymbol{w}_{j} + \boldsymbol{x}_{i}^\top \boldsymbol{\psi} \Lambda \boldsymbol{\epsilon}_{j} + \boldsymbol{\varepsilon}_{i}^\top \Lambda \boldsymbol{\epsilon}_{j}.
\end{split}
\end{equation}


This model acts as an efficient model for the fourth corner effect because it incorporates trait by environment interactions by relating them through the latent variables, rather than directly interacting with each other. Thus not all trait and species combination needs to be considered. Going through the ordination terms in, scalar notation, we have 

- $\Lambda$: the variance of the latent variable (on the link scale)
- $\sum \limits^d_{q=1}\sum \limits^p_{k=1}x_{i,k} \psi_{k,q} \Lambda_{q,q} \sum \limits^{tr}_{t=1}\delta_{t,q}w_{j,tr}$: the fourth-corner term approximated in reduced rank, or a double constrained ordination
- $\sum \limits^d_{q=1}\varepsilon_{i,q} \Lambda_{q,q}\sum \limits^{tr}_{t=1} \delta_{t,q}w_{j,t}$: a constrained ordination where the species loadings are constrained by traits alone
- $\sum \limits^d_{q=1}\sum \limits^p_{k=1} x_{i,k}\psi_{k,q}\Lambda_{q,q}\epsilon_{j,q}$: a constrained ordination with site effects determined by environmental covariates
- $\sum \limits^d_{q=1}\varepsilon_{i,q}^\top \Lambda_{qq} \epsilon_{j,q}$: a residual ordination

Each of which would have a different place in the analysis of co-occurrence patterns in ecological communities. Of course, these terms can also include structured random effects.
