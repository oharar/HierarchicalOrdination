...
## The Model
<details><summary>A quick look with the *mvabund* *R-package* indicates that the Poisson mean-variance assumption is inappropriate here.</summary>

```{r meanvar, cache=F, message=FALSE}
mvabund::meanvar.plot(Y, ylab="Sample variances (log scale)", xlab="Sample means (log scale)")
abline(a = 0, b = 1, col = "blue")
```

See [Warton (2008)](https://onlinelibrary.wiley.com/doi/pdf/10.1111/j.1442-9993.2007.01816.x) for more information.
...
$$Y_{ij} \sim \text{NB}(\lambda_{ij}, \phi_j),$$
where $\phi_j$ is the scale parameter for each species. Also,
...
<details><summary>All functions that we need for or after model fitting are here.</summary>

<details><summary>Code to generate initial values from prior distributions is here.</summary>
...
<details><summary>Functions to process results after MCMC are here.</summary>
```{r ProcessFunc}
...
<details><summary>Plotting functions.</summary>
```{r PlottingFunc}
...
<details><summary>Functions to retrieve results for the 4th corner term.</summary>
```{r EnvTraitIntFunc}
...
<details><summary>Some final summary functions.</summary>
```{r summary}
...
#                       Nburn = 5e1, NIter = 5.5e2, Nthin = 1, # for a small run
                       ...
                       consts = consts, 
                       block = MakeBlockList(consts, LVwise = TRUE), slice = c(paste0("beta0[",1:consts$NSpecies,"]"),paste0("phi[",1:consts$NSpecies,"]"))) # HMC on the rest (hypers)
```
```{r MCMCprocess, message=FALSE}
...
chains <- postProcess(chains.unsgn, VarsToProcess = VarsToProcess, 
                      VarToSign = VarsToSign, print = TRUE, rule = 2)#, VarsToSwapBy = "B[26]")
...
```{r results1, warning=FALSE, message=FALSE, fig.height = 30, cache=F}
...
```{r ResO, warning=FALSE, message=FALSE, fig.height = 30, cache=F}
...
```{r plotEnv1LV, warning=FALSE,message=FALSE, fig.height=10, cache=F}
par(mar=c(4.1, 7, 1, 1))
...
```{r plotTraits1LV, warning=FALSE,message=FALSE, fig.height=6, cache=F}
par(mar=c(4.1, 7, 1, 1))
...
```{r PlotScores, fig.height=14, cache=F}
...
To prevent rotational invariance, we require both $\boldsymbol{B}$ and $\boldsymbol{O}$ to have zeros above the main diagonal. van der Veen *et al* (2023) assumed $\boldsymbol{B}$ to be a semi-orthogonal matrix instead, as they encountered numerical instability with the constraints that we impose here. However, their constraint would require sampling on constrained parameter spaces, which is difficult, while this constraint formulation allows to use standard out-of-the-box MCMC samplers (also, it seems to work fine here). Together with the scale constraint for the latent variables there are $d(d-1)-2K$ constraints in the model, and when $d = K$ the number of parameters is the same in the interaction terms as for the fourth corner model by [Niku *et al.*(2021)](https://onlinelibrary.wiley.com/doi/full/10.1002/env.2683). When $d \gt min(K,T)$ the model can still be fitted (but either $\boldsymbol{B}$ or $\boldsymbol{O}$ needs to be padded with a column of zeros), but with $d \gt max(d,K)$ there are no further predictor effects to be estimated (as the model includes the same number of parameters as the full rank term) and additional constraints will need to be introduced.
</details>
...
<details><summary>We rotate the ordination after the MCMC to make it more interpretable. Here is an explanation of what is done.</summary>
...
# Re-scale gamma
chains2LV.sc <- RescaleChains(HO2.LV, ScaleBy = "gamma", 
...
# post-process chains for sign-swapping
chains2LV <- postProcess(chains2LV.r, VarsToProcess = VarsToProcess, print = T, rule = 2)#, VarsToSwapBy = c("1]" = "B[26, 1]", "2]" = "B[22, 2]"), print = F)
...
```{r results2, fig.height=30, cache=F}
chainsPlot(chains2LV, var = "B", legend = F, ncols = 2)
```

And the trait effects:

```{r ResO2LV, warning=FALSE,message=FALSE, fig.height=30, cache=F}
...
```{r pred.eff, fig.height=15, cache=F}
